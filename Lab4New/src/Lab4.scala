...
*/
  println("\nq_7")
  def power2 (x: Int)(n: Int)(acc: Int): Int ={
    if (n==0) acc
    else (power2(x)(n-1)(acc*x))
  }
  
  def qn_7(x:Int, n:Int) = power2(x)(n)(1)
...
*/
println("\nq_8")
def power3 (x: Int) (n: Int)(acc: Int): Int ={
    if (n==0)  acc
    else if(n%2==0) (power3(x*x)(n/2)(acc))
    else (power3(x*x)((n-1)/2)(acc*x))
  }
  
  
  def qn_8(x:Int, n:Int) = power3(x)(n)(1)
...
*/
  println("\nq_9")
  def Last_Fold_Left [X](x:List[X]) :X = 
       x match {
      case Nil => throw new Exception("no last element")
      case _ => x.foldLeft(x.head)((b,a)=> a)
      }
  def qn_9[X](x:List[X]) = Last_Fold_Left(x)
  
  println(qn_9(List(2,3,4,5,6)))
  println(qn_9(List("a","b","c","d")))
  try { 
     println(qn_9(List()))
   } catch {
...
def average (x:List[Int]) :Float = 
      ...
      case _ => x.foldLeft(0,(0).toFloat,0)((r,c) =>
    (r._1 + c,((r._1+c)/(r._3+1)), r._3+1) )._2
      }
  def qn_10(x:List[Int]) = average(x)
  
  println(qn_10(List(2,3,4,5,6)))
  try { 
     println(qn_10(List()))
   } catch {
     case e: Exception => println(e)
...
*/
  println("\nq_11")
  def map_tree [X,Y](t:Tree[X],fn:X=>Y) :Tree[Y] = 
      t match{
    case Leaf(v) => Leaf(fn(v))
    case Node(v,lt,rt) => Node(fn(v),map_tree(lt,fn),map_tree(rt,fn))
  }
  def fold_tree [X,Y](t:Tree[X],f1: X=>Y, f2:(X,Y,Y)=>Y) :Y = 
      t match{
    case Leaf(v) => f1(v)
    case Node(v,lt,rt) => f2(v,fold_tree(lt,f1,f2),fold_tree(rt,f1,f2))
  }
  def qn_11_1[X,Y](t:Tree[X],fn: X=>Y) = map_tree(t,fn)
  def qn_11_2[X,Y](t:Tree[X],f1: X=>Y, f2:(X,Y,Y)=>Y) = fold_tree(t,f1,f2)

  println(qn_11_1 (Node (3,Leaf (1), Leaf (2)),((a:Int)=> a+1)))
  println(qn_11_2 (Node (3,Leaf (1), Leaf (2)),((a:Int)=> a+1),((a:Int,b:Int,c:Int)=>a+b+c)))
