...
let align_print s1 s2 penalty_space =
	let print ac bc = if ac=bc then print_string  "+"
									else if ac=' ' ||bc=' ' then print_string  "*"
									else print_string  "-" in
	let print_result (m,a,b) = 
		print_string  (a^"\n");
		print_string  (b^"\n");
		for i = 0 to (String.length a - 1) do
			print (String.get a i)  (String.get b i);
		done;
		print_string ("\nScore : "^(string_of_int m)^"\n");
	in
	print_result (align_w_penalty s1 s2 penalty_space)

align_print "GATCGGCAT" "CAATGTGAATC" (-1);;
align_print "GATCGGCAT" "CAATGTGAATC" (-2);;
